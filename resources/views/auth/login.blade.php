...
<head>
    ...
    <title>شريكي - تسجيل الدخول</title>
</head>
...
<body class="bg-gray-100 flex justify-center items-center">
    <div class="container bg-white shadow-lg rounded-lg">
        <form method="POST" action="" class="max-w-sm mx-auto">
            @csrf
            <h1 class="text-center text-2xl mb-4">تسجيل الدخول</h1>
            @if ($errors->any())
            ...
            <a href="{{ route('password.reset') }}" class="block text-center mb-4">→ نسيت كلمة السر ←</a>
        </form>
        <hr class="w-64 h-1 bg-gradient-to-r from-transparent via-black/40 to-transparent" />
        <div class="toggle-container mt-4">
            <p class="text-center">ليس لديك حساب في منصة شريكي </p>
            <a href="{{ route('register') }}" class="overlay-btn block mx-auto mt-2">
                <button class="overlay-btn block mx-auto mt-2">سجل حساب جديد</button>
            </a>
        </div>
    </div>
